...
def setting():
    ...
    if scheduling_method == "RR":
       ...
       # print(f"\nNTT: {RR.getNTT()}")
        ...
        RQ_list = []
        RQ_list = RR.getRQ()
        for k in range(len(RQ_list)):
            Ready_Queue = Label(root, text=RQ_list[k], background = "white", font = ("",30))
            Ready_Queue.place(x=50,y=430)
            if(k <len(RQ_list)-1):
                Ready_Queue.destroy()
    ...
    if scheduling_method == "SPN":
        ...
        SPN_RQ_list = []
        SPN_RQ_list = SPN.getRQ()
        for k in range(len(SPN_RQ_list)):
            Ready_Queue = Label(root, text=SPN_RQ_list[k], background = "white", font = ("",30))
            Ready_Queue.place(x=50,y=430)
            if(k <len(SPN_RQ_list)-1):
                Ready_Queue.destroy()
...
# 스케줄링 종료시간 호출법
#print(f"\nTIME: {RR.getTIME()}")
# 각 코어(프로세서)마다 [코어 종류, 시작시간, 종료시간, 프로세스 종류] 얻는 법
#print(f"\n<PROCESSOR INFO>\n{RR.getINFO()}")
'''
