...
def setting():
    ...
    RR_time_quantum__ = eval(Entry.get(RR_time_quantum_)) # 숫자
    scheduling_method = scheduling_method_.get()
    if scheduling_method == "RR":
        print("RR선택")
        print(type(scheduling_method_.get()))
        test(Pcore_Num_,Ecore_Num_)
        
    
    
    print(scheduling_method)
    print(RR_time_quantum__)
...
def test(Pcore_Num_,Ecore_Num_):
    print("test: ",Pcore_Num_ + Ecore_Num_)
